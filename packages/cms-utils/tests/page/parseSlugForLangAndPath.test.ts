...
describe('parseSlug', () => {
  it('extracts language and the remaining path', () => {
    ...
    expect(result).toEqual({ lang: 'en', path: '/blog/post-1', pathType: 'BlogPost', pagination: undefined });
  });
  ...
  it('treats slug without language as a raw path', () => {
    ...
    expect(result).toEqual({ lang: undefined, path: 'about/contact', pathType: 'Page', pagination: undefined });
  });

  it('handles empty or null input safely', () => {
    expect(parseSlug('')).toEqual({ lang: undefined, path: '/', pathType: 'Page', pagination: undefined });
    expect(parseSlug(null)).toEqual({ lang: undefined, path: '/', pathType: 'Page', pagination: undefined });
  });
});
