 ...
 */
export const containsEnhancedImages = (container: HTMLElement | null): boolean => {
 ...
 */
export const initializeEnhancedImages = (container: HTMLElement | null): void => {
  if (!container) return;

  const enhancedImageWrappers = [
    ...Array.from(container.querySelectorAll(`[${ENHANCED_IMAGE_ATTRIBUTES.CONTAINER}]`)),
    ...Array.from(container.querySelectorAll(`.${ENHANCED_IMAGE_CLASSES.WRAPPER}`)),
  ];

  enhancedImageWrappers.forEach((wrapper) => {
    const isInline = wrapper.getAttribute(ENHANCED_IMAGE_ATTRIBUTES.INLINE) === 'true';

    const alignment = (wrapper.getAttribute(ENHANCED_IMAGE_ATTRIBUTES.ALIGNMENT) ||
